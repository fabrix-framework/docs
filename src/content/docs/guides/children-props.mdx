...
import A from "../../../assets/query-anatomy.png"
...
<Image src={A} alt="depiction of terminologies, operation is a GraphQL operation, and component is a top-level selection in the operation" />
...
>
  {({ getComponent }) => (
    <>
      <h1>Company list</h1>
      {getComponent("getUsersAndCompanies", "companies")}
    </>
  )}
</FabrixComponent>
```
